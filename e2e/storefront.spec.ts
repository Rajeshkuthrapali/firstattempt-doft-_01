...
test.describe("Storefront — Homepage", () => {
  ...
  test("brand wordmark and navigation links are visible on desktop", async ({
    page,
  }) => {
  ...
  test("value props and footer section render correctly", async ({ page }) => {
    ...
    await expect(
      valueProps.getByRole("heading", { name: "Hand-Poured" }),
    ).toBeVisible();
    await expect(
      valueProps.getByRole("heading", { name: "100% Natural Soy" }),
    ).toBeVisible();
    await expect(
      valueProps.getByRole("heading", { name: "Gift-Ready" }),
    ).toBeVisible();

    await page
      .getByRole("heading", { name: "Stay in Touch" })
      .scrollIntoViewIfNeeded();
    await expect(
      page.getByRole("heading", { name: "Stay in Touch" }),
    ).toBeVisible();
