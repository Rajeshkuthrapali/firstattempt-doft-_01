    ...
    <div>
      <div className="flex items-center justify-between mb-8">
        <div>
          ...
          <p className="text-sm text-muted mt-1">{orders.length} orders total</p>
        </div>
        <button
          onClick={exportCSV}
      ...
      <div className="overflow-x-auto border border-hairline bg-white">
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-[#f0ebe5]">
            {orders.map((o) => (
              <tr key={o.id} className="hover:bg-soft-cream transition-colors">
                <td className="px-4 py-3 font-mono text-xs text-dark">{o.id}</td>
                <td className="px-4 py-3 text-ink">{o.email}</td>
                <td className="px-4 py-3 text-center text-dark">{o.items}</td>
                <td className="px-4 py-3 font-medium text-ink">{formatPrice(o.total)}</td>
                <td className="px-4 py-3">
                  <span className={`px-2.5 py-0.5 text-[10px] font-semibold uppercase tracking-wider ${statusColors[o.status]}`}>
                    {o.status}
                  </span>
                </td>
                <td className="px-4 py-3 text-xs text-muted">{o.createdAt}</td>
                <td className="px-4 py-3">
                  <select
                    value={o.status}
                    onChange={(e) => updateStatus(o.id, e.target.value as OrderStatus)}
                    className="text-xs border border-hairline px-2 py-1 outline-none focus:border-ink bg-white text-ink"
